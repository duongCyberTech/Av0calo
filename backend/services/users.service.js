...
class UserService {
  async createUser(data) {
    try {
      const { fname, lname, username, role, status, email, password, phone } =
        data;
  ...
  async updateUser(id, data) {
    try {
      const { fname, username, lname, phone, status } = data;
      const [result] = await pool.query(
        `
                ...
                WHERE uid = ?
            `,
        [fname, lname, username , phone, status, id]
      );
  ...
  async deleteUser(id) {
    try {
      const [result] = await pool.query(`DELETE FROM users WHERE uid = ?`, [
        id,
      ]);
      return result.affectedRows > 0;
    } catch (error) {
